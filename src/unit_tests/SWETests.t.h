...
class swe_tests::SWETestsSuite : public CxxTest::TestSuite
{
    ...
    void testReadAssignment(void)
    {
        ...
        // ## Test int (valid) ##
        TS_ASSERT(SWE_CDLParser::readIntAssignment(text, string("a"), '=', reti));
        ...
        // ## Test double (valid) ##
        TS_ASSERT(SWE_CDLParser::readDoubleAssignment(text, string("b"), '=', retd));
        //Test value is as expected
        ...
        TS_ASSERT(SWE_CDLParser::readIntAssignment(text, string("charlie"), '=', reti));
        //Test text remains equal
        TS_ASSERT((*text) == " charlie = notreallyavalidnumber");

        // ## Test int (invalid): variable name is invalid ##
        TS_ASSERT(SWE_CDLParser::readIntAssignment(text, string("november"), '=', reti));
        //Test text remains equal
        TS_ASSERT((*text) == " charlie = notreallyavalidnumber");
        ...
        // ## Test double (invalid) ##
        TS_ASSERT(SWE_CDLParser::readDoubleAssignment(text, string("charlie"), '=', retd));
        //Test text remains equal
        TS_ASSERT((*text) == " charlie = notreallyavalidnumber");

        // ## Test double (invalid): variable name is invalid ##
        TS_ASSERT(SWE_CDLParser::readDoubleAssignment(text, string("november"), '=', retd));
        //Test text remains equal
        TS_ASSERT((*text) == " charlie = notreallyavalidnumber");
