  ...
  const bool i_multipleBlocks) 
{
  ...
  for(int i=0; i<=nx+1; i++) 
  {
    ...
    {
      b[i][j] = useData? i_scenario.getB(i-1, j-1) : i_scenario.getBathymetry( offsetX + (i-0.5f)*dx, offsetY + (j-0.5f)*dy );
    }
  }
  ...
  for(int i=1; i<=nx; i++)
  {
    for(int j=1; j<=ny; j++) 
    {
      ...
      h[i][j] =  useData? i_scenario.getH(i-1, j-1) : i_scenario.getWaterHeight(x,y); // - b[i][j];
      hu[i][j] = useData? i_scenario.getHu(i-1, j-1) : i_scenario.getVeloc_u(x,y) * h[i][j];
      hv[i][j] = useData? i_scenario.getHv(i-1, j-1) : i_scenario.getVeloc_v(x,y) * h[i][j]; 
    }
  }
